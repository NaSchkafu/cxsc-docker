...
//! Calculates \f$ [x]^2  \f$
       interval sqr     (const interval&) noexcept; // sqr(x)
...
//! Calculates \f$ \sqrt{1+[x]^2} \f$
       interval sqrt1px2(const interval&) noexcept; // sqrt(1+x^2)
//! Calculates \f$ \sqrt{([x]+1)-1} \f$
       interval sqrtp1m1(const interval&) noexcept; // sqrt(x+1)-1
...
inline interval sin     (const interval&) noexcept; // sin(x)
//! Calculates \f$ \mbox{sin}(\pi*x)/\pi \f$;
       interval sinpix_pi(const interval& x);      // sin(pi*x)/pi
//! Calculates \f$ \cos([x]) \f$
inline interval cos     (const interval&) noexcept; // cos(x)
//! Calculates \f$ \tan([x]) \f$
inline interval tan     (const interval&) noexcept; // tan(x)
//! Calculates \f$ \cot([x]) \f$
inline interval cot     (const interval&) noexcept; // cot(x)
...
inline interval exp     (const interval&) noexcept; // exp(x)
...
//! Calculates \f$ \ln(1+[x]) \f$
       interval lnp1    (const interval&) noexcept; // ln(1+x)
...
inline interval sinh    (const interval&) noexcept; // Sinh(x)
//! Calculates \f$ \cosh([x]) \f$
inline interval cosh    (const interval&) noexcept; // Cosh(x)
//! Calculates \f$ \tanh([x]) \f$
inline interval tanh    (const interval&) noexcept; // Tanh(x) 
//! Calculates \f$ \coth([x]) \f$
inline interval coth    (const interval&) noexcept; // Coth(x)
...
//! Calculates \f$ [x]^{[y]} \f$
interval pow     (const interval&, const interval&) noexcept;  // Pow(x,y)
//! Calculates \f$ [x]^n \f$
...
interval sqrtx2y2(const interval&, const interval&) noexcept; // sqrt(x^2+y^2)
//! Calculates \f$ \ln{\sqrt{[x]^2+[y]^2}} \f$
interval ln_sqrtx2y2(const interval&, const interval&) noexcept;
