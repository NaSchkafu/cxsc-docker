...
void rnd(const idotprecision &a,interval &b) noexcept
{
   Inf(b)=rnd(a.inf,RND_DOWN);
   Sup(b)=rnd(a.sup,RND_UP);
}

interval rnd(const idotprecision &a) noexcept
{
   interval b;
   rnd(a,b);
   return b;
...
void accumulate(idotprecision & a, const interval & b, const interval & c) noexcept
{
