...
cinterval tan( const cinterval& z ) noexcept //----------------------- 040827 --
{
  interval
      rez = Re(z),   // rez = z.re(),
      imz = Im(z);   // imz = z.im();
...
//
//
interval Atan(const interval& y, const interval& x) noexcept
...
interval Atan(const interval& y, const real& x) noexcept
// Calculating an inclusion of atan(y/x) with x<>0.0;
...
//
interval arg_inclmon( const cinterval& z ) noexcept
{
...
cinterval sqrt( const cinterval& z, int n ) noexcept // ----- 040915 --
//
...
*/
cinterval power( const cinterval& z, int n ) noexcept //---- 040720 --
//
//  Power function for integer powers with optimal (save roundoff) accuracy.
//
{
...
cinterval pow( const cinterval& z, const cinterval& p ) noexcept
{
    return exp( p*Ln(z) );
}
...
cinterval asin( const cinterval& z ) noexcept //---------------------- 040730 --
{
...
//-- acos ----------------------------------------------------------- 040730 --
//
cinterval acos( const cinterval& z ) noexcept  //--------------------- 040730 --
{
    const real gr = 6.355804e307; // upper bound for abs(rez),abs(imz)
    interval 
	rez = Re(z),
...
cinterval atan( const cinterval& z ) noexcept //----- 040912 --
{
  interval
    rez = Re(z),
    imz = Im(z);
