...
l_cinterval exp(const l_cinterval& z) noexcept
{
...
l_cinterval exp2(const l_cinterval& z) noexcept
{
	return exp(z*Ln2_l_interval());
}

l_cinterval exp10(const l_cinterval& z) noexcept
{
	return exp(z*Ln10_l_interval());
}

l_cinterval expm1(const l_cinterval& z) noexcept
// exp(z) - 1;
...
l_cinterval cos(const l_cinterval& z) noexcept
{
    int stagsave = stagprec,
	stagmax = 19;
    l_interval lreal(Re(z)),limg(Im(z));
...
l_cinterval sin(const l_cinterval& z) noexcept
{
    int stagsave = stagprec,
	stagmax = 19;
    l_interval lreal(Re(z)),limg(Im(z));
...
l_cinterval cosh(const l_cinterval& z) noexcept
{
    int stagsave = stagprec,
	stagmax = 19;
    l_interval lreal(Re(z)),limg(Im(z));
...
l_cinterval sinh(const l_cinterval& z) noexcept
{
    int stagsave = stagprec,
	stagmax = 19;
    l_interval lreal(Re(z)),limg(Im(z));
...
l_cinterval sqr(const l_cinterval& z) noexcept
...
l_cinterval Tan( const l_cinterval& z ) noexcept // -------------- 040827 --
{
    l_cinterval y;
...
l_cinterval tan( const l_cinterval& z ) noexcept 
{
...
l_cinterval cot( const l_cinterval& z ) noexcept 
{
...
//  tanh( z ) = transp( i * tan( transp( i * z ) )
//
l_cinterval tanh( const l_cinterval& z ) noexcept
{
    int stagsave = stagprec,
	stagmax = 19; 
    if (stagprec < stagmax) stagprec++;
...
l_cinterval coth( const l_cinterval& z ) noexcept
{ // coth( z ) = i * cot( i * z );
    int stagsave = stagprec,
	stagmax = 19; 
    if (stagprec < stagmax) stagprec++;
...
l_interval Atan(const l_interval& y, const l_real& x) noexcept
// Calculating an inclusion of atan(y/x) with x<>0.0;
...
//
l_interval Arg( const l_cinterval& z ) noexcept
{
    l_real 
	srez = Sup( Re(z) ),
	irez = Inf( Re(z) ), 
...
l_interval arg_inclmon( const l_cinterval& z ) noexcept
{
...
//  axis from below;
//
l_cinterval Ln( const l_cinterval& z ) noexcept
{ // Blomquist;
    int stagsave = stagprec,
	stagmax = 19; 
    if (stagprec < stagmax) stagprec++;
...
l_cinterval lnp1(const l_cinterval& z) noexcept
{  // ln(1+z);
	// Calculates nearly optimal inclusions for not too wide intervals z.
   // Blomquist, 03.12.2008;
	int stagsave = stagprec,
...
l_cinterval log2( const l_cinterval& z ) noexcept
{
	return Ln(z) / Ln2_l_interval();
}

l_cinterval log10( const l_cinterval& z ) noexcept
{
...
l_cinterval sqrt( const l_cinterval& z ) noexcept // ----------------------
//
...
l_cinterval sqrtp1m1(const l_cinterval& z) noexcept
// sqrt(1+z)-1;
// Calculates nearly optimal inclusions for not too wide intervals z.
...
l_cinterval sqrt1px2(const l_cinterval& z) noexcept
// sqrt(1+z^2);
// Calculates nearly optimal inclusions for not too wide intervals z.
...
l_cinterval sqrtx2m1(const l_cinterval& z) noexcept
// sqrt(z^2-1);
// Calculates nearly optimal inclusions for not too wide intervals z.
...
l_cinterval sqrt1mx2(const l_cinterval& z) noexcept
// sqrt(1-z^2);
// Calculates nearly optimal inclusions for not too wide intervals z.
// Blomquist, 04.12.2008;
{
...
l_cinterval sqrt( const l_cinterval& z, int n ) noexcept // ----- 040915 --
//
...
//
l_cinterval power_fast( const l_cinterval& z, int n ) noexcept
{
    if( n == 0 )
	return l_cinterval( l_interval(1) );
    else if( n == 1 )
...
l_cinterval power( const l_cinterval& z, int n ) noexcept //---- 040720 --
//
//  Power function for integer powers with optimal (save roundoff) accuracy.
//
{
...
l_cinterval pow( const l_cinterval& z, const l_interval& p ) noexcept
...
l_cinterval pow( const l_cinterval& z, const l_cinterval& p ) noexcept
{
    return exp( p*Ln(z) );
}
...
l_cinterval asin( const l_cinterval& z ) noexcept //------------- 040730 --
{
...
//-- acos ---------------------------------------------------
//
l_cinterval acos( const l_cinterval& z ) noexcept
{
    const real gr = 6.355804e307; // upper bound for abs(rez),abs(imz)
    l_interval 
	rez = Re(z),
...
//-- asinh ---------------------------------------------------------- 040730 --
//
l_cinterval asinh( const l_cinterval& z ) noexcept
//
//  asinh( z ) = i * asin( -i * z )
//
{
...
//-- acosh ---------------------------------------------------------- 040908 --
//
l_cinterval acosh( const l_cinterval& z ) noexcept
//
//  acosh( z ) = i * acos( z ) = +/- i * ( pi / 2 - asin( z ) )
//
{
...
l_cinterval atan( const l_cinterval& z ) noexcept //----- 040912 --
{
    l_interval
	rez = Re(z),
	imz = Im(z);
...
//
l_cinterval acot( const l_cinterval& z ) noexcept
{
 l_interval
    rez = Re(z),
    imz = Im(z);
...
//-- atanh ---------------------------------------------------------- 040912 --
//
l_cinterval atanh( const l_cinterval& z ) noexcept
//
//  atanh( z ) = - i * atan( i * z )
//
{
...
//-- acoth ---------------------------------------------------------- 040912 --
//
l_cinterval acoth( const l_cinterval& z ) noexcept
//
//  acoth( z ) = i * acot( i * z )
//
{
