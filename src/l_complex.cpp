...
cdotprecision operator+(const l_complex& lc, const cdotprecision& cd) noexcept 
{ 
   return _cdotprecision(lc) + cd; 
}

cdotprecision operator+(const cdotprecision& cd, const l_complex& lc) noexcept 
{ 
   return _cdotprecision(lc) + cd; 
}   
...
cdotprecision operator+(const l_complex& lc, const dotprecision& cd) noexcept 
{ 
   return _cdotprecision(lc) + cd; 
}

cdotprecision operator+(const dotprecision& cd, const l_complex& lc) noexcept 
{ 
   return _cdotprecision(lc) + cd; 
}   
...
l_complex operator / (const l_complex &a, const l_complex &b) noexcept
{
   return divn(a,b);
}

int StagPrec(const l_complex& lc) noexcept
{
    return StagPrec(lc.re);
}      

void accumulate(cdotprecision& cd, const l_complex& lc1, 
                                   const l_complex& lc2) noexcept
{
...
void accumulate(cdotprecision& cd, const l_complex& lc, 
                                   const l_real& lr) noexcept
{
    accumulate(Re(cd),lc.re,lr); 
    accumulate(Im(cd),lc.im,lr);
}

l_real abs2(const l_complex &a) noexcept
{
   dotprecision dot(0.0);
...
l_real abs(const l_complex& z) noexcept
...
complex & complex::operator = (const l_complex& a) noexcept
{
	real x(Re(a)), y(Im(a));
