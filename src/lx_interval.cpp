...
lx_interval::lx_interval(const real& n, const string &s) noexcept
// Constructor:
...
l_interval & l_interval::operator = (const lx_interval &a) noexcept
{
    int exa;
    real p(expo(a)),exr;
...
interval & interval::operator = (const lx_interval &a) noexcept
{
    l_interval al;
    interval z;
    al = a;
...
lx_interval::lx_interval(const lx_real& a, const lx_real& b) noexcept
{
    lx_real a_(a),b_(b);
    l_real lr;
    l_interval h;
...
lx_interval operator + (const lx_interval &a, const lx_interval &b) noexcept
{
...
void operator >> (const char *s, lx_interval &a) noexcept
{
    std::string r(s);
    r >> a;
}
...
std::istream & operator >> (std::istream &s, lx_interval &a) noexcept
...
// ----------------------------------------------------------------------------
	...
	lx_real::lx_real(const real& n, const string &s) noexcept
// Constructor:
...
// (ex,li) is an approximation of  10^n*s;
{
	...
	std::string & operator >> (std::string &s, lx_real &a) noexcept
...
// with:    10^(-4000)*2 ~ a;
{
	...
	void operator >> (const std::string &s, lx_real &a) noexcept
{
   // Writes string s to variable a of type lx_real;
	std::string r(s);
	r >> a;
	...
	void operator >> (const char *s, lx_real &a) noexcept
{
	std::string r(s);
	r >> a;
}	
...
bool operator == (const lx_real &a, const lx_real &b) noexcept
{
	const real c1 = 1e20;
...
void scale_up(lx_real &a) noexcept
...
lx_real operator + (const lx_real &a, const lx_real &b) noexcept
...
l_real & l_real::operator = (const lx_real& a) noexcept
{
	int exar;
	real p(expo(a)), exr;
	l_real ar(lr_part(a));
...
real & real::operator = (const lx_real& a) noexcept
{
	l_real lr;
	real x;
...
lx_interval sqrt(const lx_interval &a) noexcept
{
	int stagsave = stagprec,
...
lx_interval sqr(const lx_interval &x) noexcept
{
	int stagsave = stagprec,
 stagmax = 39;
 if (stagprec>stagmax) stagprec = stagmax;
...
lx_interval Lnp1(const lx_interval &x) noexcept
...
lx_interval Ln_(const lx_interval &x) noexcept 
{
	lx_interval T(0.0);
...
lx_interval ln(const lx_interval &x) noexcept 
{
	int stagsave = stagprec,
 stagmax = 39;
 if (stagprec>stagmax) stagprec = stagmax;
...
lx_interval log2(const lx_interval &x) noexcept
{
	int stagsave = stagprec,
 stagmax = 39;
 if (stagprec>stagmax) stagprec = stagmax;
...
lx_interval log10(const lx_interval &x) noexcept
{
	int stagsave = stagprec,
 stagmax = 39;
 if (stagprec>stagmax) stagprec = stagmax;
...
lx_interval Lnp1_(const lx_interval &x) noexcept
{
	const real r = 1e-7;
	lx_interval res;
...
lx_interval Power_(const lx_interval &x, const real &n) noexcept
...
lx_interval power(const lx_interval &x, const real &n) noexcept
{
	int stagsave = stagprec,
 stagmax = 38;
 if ( !(Is_Integer(n)) ) 
...
lx_interval Exp_(const lx_interval &x) noexcept
{
	int ex,n,N;
...
lx_interval exp(const lx_interval &x) noexcept
{
	int stagsave = stagprec,
 stagmax = 40;
...
lx_interval exp2(const lx_interval &x) noexcept
{
	int stagsave = stagprec,
 stagmax = 40;
 if (stagprec>stagmax) stagprec = stagmax;
...
lx_interval exp10(const lx_interval &x) noexcept
{
	int stagsave = stagprec,
 stagmax = 40;
 if (stagprec>stagmax) stagprec = stagmax;
...
lx_interval pow(const lx_interval &x, const lx_interval &e) noexcept
...
// Rounding to the next integer number z, with z <= x;
 ...
 lx_interval sin(const lx_interval &x) noexcept
// Inclusion of sin(x)
...
//    if (n%2 != 0) res = -res;
 ...
 lx_interval cos(const lx_interval &x) noexcept
// Inclusion of cos(x)
 {
	 int stagsave = stagprec,
  stagmax = 39;
...
//    if (n%2 == 0) res = -res;
 ...
 lx_interval tan(const lx_interval& x) noexcept 
 {
	 lx_interval c,y;
 ...
 lx_interval cot(const lx_interval & x) noexcept 
 {
	 lx_interval s,y;
 ...
 lx_interval sqrt1px2(const lx_interval &x) noexcept 
...
// Only for the internal use in the Atan_-function.
 ...
 lx_interval Atan_(const lx_interval &x) noexcept 
...
// Only for the internal use in atan(x)!
 ...
 lx_interval atan(const lx_interval &x) noexcept
 {
	 int stagsave = stagprec,
  stagmax = 39;
...
// ---------------------- sqrt(1-x^2) -----------------------------------
 ...
 lx_interval Sqrt1mx2_(const lx_interval &x) noexcept 
...
// Blomquist, 08.04.2008;
 ...
 lx_interval sqrt1mx2(const lx_interval &x) noexcept
 {
	 int stagsave = stagprec,
  stagmax = 39;
...
// ------------------- sqrt(x^2-1) ------------------------------------
 ...
 lx_interval sqrtx2m1(const lx_interval &x) noexcept 
...
// ------------------- asin(x) ----------------------------------------
 ...
 lx_interval Asin_(const lx_interval& x) noexcept
...
// Only for the internal use in asin(x)!
 ...
 lx_interval asin(const lx_interval &x) noexcept
 {
	 int stagsave = stagprec,
  stagmax = 39;
...
// ------------------- acos(x) ----------------------------------------
 ...
 lx_interval Acos_(const lx_interval& x) noexcept
...
// Blomquist, 24.04.2008;
 ...
 lx_interval acos(const lx_interval &x) noexcept
// Blomquist, 24.04.2008;
 {
	 int stagsave = stagprec,
  stagmax = 39;
...
// ------------------- acot(x) ----------------------------------------
 ...
 lx_interval Acot_(const lx_interval& x) noexcept
...
// Blomquist, 25.04.2008; 
 ...
 lx_interval acot(const lx_interval &x) noexcept
// Blomquist, 24.04.2008;
 {
	 int stagsave = stagprec,
  stagmax = 39;
...
// -------------------------- sinh(...) --------------------------------------
 ...
 lx_interval Sinh_(const lx_interval &x) noexcept
// Inclusion of sinh(x) for sufficiently tight intervals x
 ...
 lx_interval sinh(const lx_interval &x) noexcept
 {
	 int stagsave = stagprec,
  stagmax = 39;
...
// -------------------------- cosh(...) ------------------------------------
 ...
 lx_interval Cosh_(const lx_interval& x) noexcept
...
// Blomquist, 26.04.2008; 
 ...
 lx_interval cosh(const lx_interval &x) noexcept
 {
	 int stagsave = stagprec,
  stagmax = 39;
...
// -------------------------- tanh(...) ------------------------------------
 ...
 lx_interval Tanh_(const lx_interval& x) noexcept
 {
// Blomquist, 27.04.2008;
 ...
 lx_interval tanh(const lx_interval &x) noexcept
 {
	 int stagsave = stagprec,
  stagmax = 39;
...
// -------------------------- coth(...) ------------------------------------
 ...
 lx_interval Coth_(const lx_interval& x) noexcept
// This function is only used for tight intervals x,
// with Inf(x) > 0;
// Blomquist, 27.04.2008;
 {
 ...
 lx_interval coth(const lx_interval &x) noexcept
// Blomquist, 27.04.2008;
 {
	 int stagsave = stagprec,
  stagmax = 39;
...
// -------------------------- sqrt(1+x)-1 ------------------------------------
 ...
 lx_interval Sqrtp1m1_(const lx_interval& x) noexcept
...
// Blomquist, 25.08.07;
 ...
 lx_interval sqrtp1m1(const lx_interval &x) noexcept
 {
	 int stagsave = stagprec,
  stagmax = 30;
...
// -------------------------- asinh(x) ------------------------------------
 ...
 lx_interval Asinh_(const lx_interval &x) noexcept
// Inclusion of asinh(x)
 ...
 lx_interval asinh(const lx_interval &x) noexcept
 {
	 int stagsave = stagprec,
  stagmax = 39;
  if (stagprec>stagmax) 
...
// -------------------------- acosh(...) -------------------------------
 ...
 lx_interval acosh(const lx_interval &x) noexcept
...
// ------------------------- acoshp1(...) -----------------------------
 ...
 lx_interval Acoshp1_(const lx_interval &x) noexcept
// This function is only used for tight intervals x,
// with Inf(x) >= 0;
// Blomquist, 03.05.2008;
 ...
 lx_interval acoshp1(const lx_interval &x) noexcept
...
// ------------------------ atanh(...) ----------------------------------
 ...
 lx_interval Atanh_(const lx_interval &x) noexcept
...
// Blomquist, 03.05.2008;
 ...
 lx_interval atanh(const lx_interval &x) noexcept
 {
	 int stagsave = stagprec,
  stagmax = 39;
  if (stagprec>stagmax) 
...
// ------------------------ atanh1m(...) ---------------------------------
 ...
 lx_interval Atanh1m_(const lx_interval &x) noexcept
...
// ------------------------ acoth(...) ---------------------------------
 ...
 lx_interval Acoth_(const lx_interval& x) noexcept
...
//  Blomquist, 13.05.2008;
 ...
 lx_interval acoth(const lx_interval &x) noexcept
// Blomquist, 13.05.2008;
 {
	 int stagsave = stagprec,
  stagmax = 39;
...
// ------------------------ acoth(1+x) ---------------------------------
 ...
 lx_interval Acothp1_(const lx_interval& x) noexcept
//  arcoth(1+x), x>0; 
...
// ------------------------ sqrt(x^2+y^2) ------------------------------
 ...
 lx_interval Sqrtx2y2_(const lx_interval &x, const lx_interval &y) noexcept
...
// Blomquist, 15.05.2008;
 ...
 lx_interval sqrtx2y2(const lx_interval &x, const lx_interval &y) noexcept
 {
	 int stagsave = stagprec,
  stagmax = 30,
  exyl,exxl;
...
// ---------------------- ln(sqrt(x^2+y^2)) ----------------------------
 ...
 lx_interval Ln_sqrtx2y2_(const lx_interval &x, const lx_interval &y) noexcept
...
// Blomquist, 19.05.2008;
 ...
 lx_interval ln_sqrtx2y2(const lx_interval &x, const lx_interval &y) noexcept
 {
	 int stagsave = stagprec,
  stagmax = 30;
  if (stagprec>stagmax) stagprec = stagmax;
...
// -------------------------- n-th root --------------------------------
 ...
 lx_interval Sqrt_(const lx_interval& x, int n) noexcept
 ...
 lx_interval sqrt(const lx_interval &x, int n) noexcept
 {
	 int stagsave = stagprec,
  stagmax = 39,
  k(n);
...
// --------------------------------------------------------------------------
 ...
 lx_real sqrt(const lx_real& x) noexcept
 { return mid(sqrt(lx_interval(x))); }
 lx_real sqr(const lx_real& x) noexcept
 { return mid(sqr(lx_interval(x))); }
 lx_real ln(const lx_real& x) noexcept
 { return mid(ln(lx_interval(x))); }
 lx_real log2(const lx_real& x) noexcept
 { return mid(log2(lx_interval(x))); }
 lx_real log10(const lx_real& x) noexcept
 { return mid(log10(lx_interval(x))); }
 lx_real lnp1(const lx_real& x) noexcept
 { return mid(lnp1(lx_interval(x))); }
 lx_real exp(const lx_real& x) noexcept
 { return mid(exp(lx_interval(x))); }
 lx_real exp2(const lx_real& x) noexcept
 { return mid(exp2(lx_interval(x))); }
 lx_real exp10(const lx_real& x) noexcept
 { return mid(exp10(lx_interval(x))); }
 lx_real expm1(const lx_real& x) noexcept
 { return mid(expm1(lx_interval(x))); }
 lx_real power(const lx_real& x, const real& r) noexcept
 { return mid(power(lx_interval(x),r)); }
 lx_real pow(const lx_real& x, const lx_real& y) noexcept
 { return mid(pow(lx_interval(x),lx_interval(y))); }
 lx_real xp1_pow_y(const lx_real& x, const lx_real& y) noexcept
 { return mid(xp1_pow_y(lx_interval(x),lx_interval(y))); }
 lx_real sin(const lx_real& x) noexcept
 { return mid(sin(lx_interval(x))); }
 lx_real sin_n(const lx_real& x, const real& n) noexcept
 { return mid(sin_n(lx_interval(x),n)); }
 lx_real cos(const lx_real& x) noexcept
 { return mid(cos(lx_interval(x))); }
 lx_real cos_n(const lx_real& x, const real& n) noexcept
 { return mid(cos_n(lx_interval(x),n)); }
 lx_real tan(const lx_real& x) noexcept
 { return mid(tan(lx_interval(x))); }
 lx_real cot(const lx_real& x) noexcept
 { return mid(cot(lx_interval(x))); }
 lx_real sqrt1px2(const lx_real& x) noexcept
 { return mid(sqrt1px2(lx_interval(x))); }
 lx_real atan(const lx_real& x) noexcept
 { return mid(atan(lx_interval(x))); }
 lx_real sqrt1mx2(const lx_real& x) noexcept
 { return mid(sqrt1mx2(lx_interval(x))); }
 lx_real sqrtx2m1(const lx_real& x) noexcept
 { return mid(sqrtx2m1(lx_interval(x))); }
 lx_real asin(const lx_real& x) noexcept
 { return mid(asin(lx_interval(x))); }
 lx_real acos(const lx_real& x) noexcept
 { return mid(acos(lx_interval(x))); }
 lx_real acot(const lx_real& x) noexcept
 { return mid(acot(lx_interval(x))); }
 lx_real sinh(const lx_real& x) noexcept
 { return mid(sinh(lx_interval(x))); }
 lx_real cosh(const lx_real& x) noexcept
 { return mid(cosh(lx_interval(x))); }
 lx_real tanh(const lx_real& x) noexcept
 { return mid(tanh(lx_interval(x))); }
 lx_real coth(const lx_real& x) noexcept
 { return mid(coth(lx_interval(x))); }
 lx_real sqrtp1m1(const lx_real& x) noexcept
 { return mid(sqrtp1m1(lx_interval(x))); }
 lx_real asinh(const lx_real& x) noexcept
 { return mid(asinh(lx_interval(x))); }
 lx_real acosh(const lx_real& x) noexcept
 { return mid(acosh(lx_interval(x))); }
 lx_real acoshp1(const lx_real& x) noexcept
 { return mid(acoshp1(lx_interval(x))); }
 lx_real atanh(const lx_real& x) noexcept
 { return mid(atanh(lx_interval(x))); }
 lx_real atanh1m(const lx_real& x) noexcept
 { return mid(atanh1m(lx_interval(x))); }
 lx_real atanhm1p(const lx_real& x) noexcept
 { return mid(atanhm1p(lx_interval(x))); }
 lx_real acoth(const lx_real& x) noexcept
 { return mid(acoth(lx_interval(x))); }
 lx_real acothp1(const lx_real& x) noexcept
 { return mid(acothp1(lx_interval(x))); }
 lx_real acothm1m(const lx_real& x) noexcept
 { return mid(acothm1m(lx_interval(x))); }
 lx_real sqrtx2y2(const lx_real& x, const lx_real& y) noexcept
 { return mid(sqrtx2y2(lx_interval(x),lx_interval(y))); }
 lx_real ln_sqrtx2y2(const lx_real& x, const lx_real& y) noexcept
 { return mid(ln_sqrtx2y2(lx_interval(x),lx_interval(y))); }
 lx_real sqrt(const lx_real& x, int n) noexcept
